...
> `.atelier\projects\coding\gakkari-ol\`. Clock out per the charter when done.
...
- **Archive view (`v`) + resume (`r`):** `v` cycles cancelled subs in/out of the visible list, dimmed throughout so they read as for-reference (title-bar indicator `· +archive`); `r` (`action_resume`) un-cancels the highlighted cancelled sub (status → `active`), a no-op otherwise. Editing an archived sub is supported — `SubscriptionModal._status_options` adds `cancelled` to the status `Select` only for an already-cancelled sub, so the value is valid instead of raising `InvalidSelectValueError`. Cancelled rows are **never** surfaced on the notice board, even when the archive is visible — the board is the "what's actually going to charge you" surface, and a cancelled sub will not actually renew.
...
- **Notice panel (`n` cycles):**
  - **Notices state (default):** banner + 7-to-14 stacked posts (`{n} ：OL ：YYYY-MM-DD(月) ID:hash8`, body, kaomoji, `─` rule). Three pools (renewal / trial / empty) picked deterministically by `post_id` hash so the same day always renders identically. Always-JA single-char weekday labels; EN/JA bodies. Adaptive window via `_pick_window(height)`. 60-second `set_interval` tick watches for date rollover. Cancelled subs are dropped at the data layer in `build_notice_posts` as a defensive backstop, and the call site also passes the visible `_subs` list (post-archive, post-paused) so a cancelled row never appears on the right when it isn't on the left.
...
### 0.4.2 deltas — notice board respects status

- **Cancelled subs no longer appear on the notice board.** The right-column board was reading from the unfiltered `_all_subs` (kept in memory for the `v` archive toggle) and passing it straight to `build_notice_posts`, so a cancelled sub with a stale `next_renewal_date` in the future still emitted a "renews today!" post. Fixed in two layers: `build_notice_posts` now drops `status == "cancelled"` defensively (its old docstring was the load-bearing "assumed already filtered" contract, now baked in instead of trusted), and `MainScreen._refresh_notice_panel` now passes the visible `_subs` list (post-archive, post-paused) instead of `_all_subs`. Belt and braces — the data-layer guard catches any future caller that forgets the contract; the call-site swap means the notice board follows the same `v` / `p` toggles as the visible list.
- **New tests:** `tests/test_notices.py` (8 cases) covers cancelled-emits-no-post, paused-and-active-still-emit, mixed-statuses-drop-only-cancelled, cancelled-within-window-still-ignored, cancelled-with-far-future-still-ignored, trial-on-cancelled-suppressed, trial-on-active-still-fires, and a mixed-statuses-bundling assertion. No prior test coverage existed for `notices.py`.
